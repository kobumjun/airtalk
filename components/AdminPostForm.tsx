...
export default function AdminPostForm({ onSuccess }: Props) {
  ...
  return (
    <form onSubmit={handleSubmit} className="space-y-4">
      {error && (
        <div className="p-3 bg-red-900/60 border border-red-700 rounded-xl text-red-200 text-sm">
          {error}
        </div>
      )}
      {success && (
      ...
      <div>
        <label className="block text-sm font-medium text-gray-100 mb-1">제목 *</label>
        <input
          type="text"
          value={title}
